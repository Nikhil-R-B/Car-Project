...
/*void drawRadar(){
  fill(0);                              // set the following shapes to be black
  noStroke();                           // set the following shapes to have no outline
  ellipse(radius, 900, 1700, 1700);    // draw a circle with a width/ height = 750 with its center position (x and y) set by the radius
  rectMode(CENTER);                     // set the following rectangle to be drawn around its center
  rect(950,500,2000,1000);              // draw rectangle (x, y, width, height)
  for (int i = 0; i <=8; i++){
    noFill();
    strokeWeight(1);
    stroke(0, 255-(25*i), 0);
    ellipse(radius+50, 900, (200*i), (200*i)); 
    fill(0, 100, 0);
    noStroke();
    radarDist+=50;
  }
  ...
  /*for (int i = 0; i <= 6; i++) {
    strokeWeight(1);
    stroke(0, 55, 0);
    line(radius+50, radius, radius+50 + cos(radians(180+(30*i)))*w, radius + sin(radians(180+(30*i)))*w);
    fill(0, 55, 0);
    noStroke();
    if ((30*i) > 90) {
      text(Integer.toString((30*i)), (radius+20) + cos(radians((30*i)))*(w), (radius+20) + sin(radians(180+(30*i)))*(w), 100,150);
    }
    else if ((30*i) == 60){
      text("60", (radius+90) + cos(radians((30*i)))*(w), (radius+20) + sin(radians(180+(30*i)))*(w), 100,150);
    }
    else if ((30*i) == 90){
      text("90", (radius+70) + cos(radians((30*i)))*(w), (radius+20) + sin(radians(180+(30*i)))*(w), 100,150);
    }
    else {
      text(Integer.toString((30*i)), (radius+150) + cos(radians((30*i)))*(w+10), (radius+50) + sin(radians(180+(30*i)))*(w+10), 200,200);
    }
  }
}*/
